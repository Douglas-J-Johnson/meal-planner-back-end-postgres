...
# meat = List.create(name: "Meat", list_entry_id: 0)
dairy = List.new(name: "Dairy", list_entry_id: [milk_in_list.id, half_and_half_in_list.id])
...
week = Week.create(day_id: [sun.id, mon.id, tues.id, wed.id, thurs.id, fri.id, sat.id], list_id: produce.id)